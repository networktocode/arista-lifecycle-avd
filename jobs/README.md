...
| --- | --- | --- |
| Arista Lifecycle Workshop | Load AVD Change Report | Installs the Reports-app template from `reports/avd_change_report/` in this checkout (idempotent; `overwrite` rebuilds it). |
| Arista Lifecycle Workshop | 06 Post Change Validation | After a change: post snapshot with the pre snapshot's devices and rules → Operational Compliance comparison → publish the AVD Change Report. Needs nautobot-operational-compliance, nautobot-reports and nautobot-tools. |
